...
const SoundPosition_1 = require("./SoundPosition");
...
class Sound {
    ...
    constructor(src) {
        ...
        this.saveOffset = 0;
        // 3D Space:
        this._pos = new SoundPosition_1.SoundPosition();
        // Creating Main Nodes:
        this.createNodes();
        this.makeSourceNode();
    }
    ...
    get samples() {
        ...
        if (this._samples.length == 0) {
            var chanls = this.buffer.numberOfChannels, result = [];
    ...
    createNodes() {
        var ctx = this.context, gain, panner, stero;
        gain = ctx.createGain();
     ...
     */
    unlinkNodes() {
        var nodesToUnlink = [this.context.destination, this.gain, this.steroPanner, this.panner];
    ...
    linkNodes() {
        this.unlinkNodes();
        var nodesToLink = [this.context.destination, this.gain, this.steroPanner, this.panner];
    ...
    async getSoundBuffer() {
        return new Promise((resolve, reject) => {
            ...
            if (source instanceof ArrayBuffer) {
                ...
                var lnk = window.location.origin + "/" + source;
    ...
    // Sound Source Making:
    async makeSourceNode() {
        var buffer = (await this.getSoundBuffer()), ctx = this.context, source;
        if (this.source) {
            this.unlinkNodes();
            this.source = null;
        }
